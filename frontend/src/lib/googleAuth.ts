...
export function signInWithGoogle(appName = 'this app') {
  ...
  // Always use window.open() — never window.location.href, so the preview
  // iframe is never navigated away.
...
// Redirect fallback: if the popup's postMessage path fails, the callback
// redirects back to this origin with ?google_id_token=... — sign in and close.
